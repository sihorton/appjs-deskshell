				...
				if (!err) {
					...
					if (!chromeDebugUrl) {
						if (tries > 2) {
							...
							shellApi.getDebugSocket(app,appUrl,launching,2000,tries+1);
						}
					} else {
				...
				if (app.params['htdocs']) {
					...
					app.server = require('http').createServer(myApp.params['serverHandler'] || function handler (req, res) {
						...
						if (continue) {
							...
							deskShell.appfs.readFile(serveFile,undefined,function (err, buffer) {
								if (err) {
									//try default location
									mimetype = mime.lookup(serveFile+ myApp.params['defaultLocation']);
									deskShell.appfs.readFile(serveFile + myApp.params['defaultLocation'],null,function (err, buffer) {
										if (err) {
											res.writeHead(500);
