...
var shellApi = {
	...
	loadEnv:function() {
		...
		try {
			...
			deskShell.installDir = path.normalize(deskShell.platformDir);
			deskShell.sysAppsDir = path.normalize(deskShell.platformDir + deskShell.sysAppsDir);
			...
			//create it..
			var os = require('os');
			fs.readFile(deskShell.envPath+".sample."+os.platform(), 'utf8', function (err, data) {
				if (err) return loadingenv.reject(err);
				deskShell.env = JSON.parse(data);
				deskShell.installDir = path.normalize(deskShell.platformDir + deskShell.installDir);
				deskShell.sysAppsDir = path.normalize(deskShell.platformDir + deskShell.sysAppsDir);
				...
				if (!err) {
					...
					if (!chromeDebugUrl) {
						if (tries > 2) {
							...
							shellApi.getDebugSocket(app,appUrl,launching,3000,tries+1);
						}
					} else {
			...
			.then(function(dat) {
				...
				if (app.params['htdocs']) {
					...
					app.server = require('http').createServer(myApp.params['serverHandler'] || function handler (req, res) {
					    ...
					    fs.readFile(serveFile,function (err, buffer) {
							if (err) {
								//try default location
								mimetype = mime.lookup(serveFile+ myApp.params['defaultLocation']);
								fs.readFile(serveFile + myApp.params['defaultLocation'],function (err, buffer) {
									if (err) {
										res.writeHead(500);
				...
				if (app.params['openAppWin']) {
					...
					.then(function(dat) {
						...
						if (shellApi.appDef['mode'] && shellApi.appDef['mode'] == 'kiosk') {
							...
							console.log("Starting via: ",app.params['chromiumPath']);
							app.chromium = require('child_process')
						...
						shellApi.getDebugSocket(app,appUrl,launching,2000);
					});
				} else {
					launching.resolve(app);
				}
	...
	launchApp:function(path) {
		...
		require('child_process')
			.exec(deskShell.platformDir+deskShell.env.deskShellExe+' "'+path+'"',function(error, stdout, stderr) {
	...
	launchAppDebug:function(path) {
		...
		console.log(path);
		require('child_process')
			.exec(deskShell.platformDir+deskShell.env.deskShellExeDebug+' "'+path+'"',function(error, stdout, stderr) {
				if (error) console.log("launchApp exec error:"+error);
		});
	},
