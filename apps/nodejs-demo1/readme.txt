...
1) app.nsr opens a port and serves the contents of "content/" directory, navigating to /page1.htm will display the contents of content/page1.htm. This provides simple webserver functionality. In this simple demo clicking on the links will navigate you from page to page.

2) A socket is created by app.nsr this socket is connected to from the demo application. Two way communication is then possible by sending messages from the application to the server.
...
3) app.nsr opens the chrome browser so it can pass parameters to it.
